...
public class AdministratorSchirm extends JFrame implements QuizFenster, BackendWindow, ActionListener, Observer {
	...
	protected ImageIcon					windowedIcon						= new ImageIcon ( this.getClass ()
																					.getResource ( "/img/windowed.png" ) );
	protected ImageIcon					fullscreenIcon						= new ImageIcon ( this.getClass ()
																					.getResource (
																							"/img/fullscreen.png" ) );
	...
	protected JButton					spielPausierenBtn					= new JButton (
																					...
																					pauseIcon );
	protected JButton					vollbildToggleBtn					= new JButton (
																					"<html>Vollbildmodus</html>",
																					fullscreenIcon );
	...
	private void doInitialisations () {
		...
		addComponent ( quizSteuerungPanel, gbl_quizSteuerung, this.fragestellung, 0, 1, 2, 1, 1.0, 1.0, new Insets (
				20, 5, 20, 5 ) );
		...
		addComponent ( quizSteuerungPanel, gbl_quizSteuerung, antwort1Panel, 0, 2, 1, 1, 0.1, 0.2, new Insets ( 2, 5,
				2, 2 ) );

		this.antwort2Panel = new JPanel ( new GridLayout ( 1, 0 ) );
		this.antwort2Panel.add ( this.antwort2KlickenBtn );
		addComponent ( quizSteuerungPanel, gbl_quizSteuerung, antwort2Panel, 1, 2, 1, 1, 0.1, 0.2, new Insets ( 2, 2,
				2, 5 ) );

		this.antwort3Panel = new JPanel ( new GridLayout ( 1, 0 ) );
		this.antwort3Panel.add ( this.antwort3KlickenBtn );
		addComponent ( quizSteuerungPanel, gbl_quizSteuerung, antwort3Panel, 0, 3, 1, 1, 0.1, 0.2, new Insets ( 2, 5,
				2, 2 ) );

		this.antwort4Panel = new JPanel ( new GridLayout ( 1, 0 ) );
		this.antwort4Panel.add ( this.antwort4KlickenBtn );
		addComponent ( quizSteuerungPanel, gbl_quizSteuerung, antwort4Panel, 1, 3, 1, 1, 0.1, 0.2, new Insets ( 2, 2,
				2, 5 ) );

		addComponent ( quizSteuerungPanel, gbl_quizSteuerung, new JLabel ( "Bibelstelle (Tipp-Joker): " ), 0, 4, 1, 1,
				0, 0, new Insets ( 5, 5, 5, 5 ) );
		addComponent ( quizSteuerungPanel, gbl_quizSteuerung, bibelstelleLabel, 1, 4, 1, 1, 0, 0, new Insets ( 5, 5, 5,
				5 ) );
		...
		addComponent ( this.dateiPanel, gbl_dateiPanel, this.neuesSpielAusInternHerunterladenBtn, 2, 3, 1, 1, 0, 0,
				new Insets ( 0, 2, 2, 2 ) );
		addComponent ( this.dateiPanel, gbl_dateiPanel, this.neuesStandardSpielStartenBtn, 2, 4, 1, 1, 0, 0,
				new Insets ( 0, 2, 2, 2 ) );
		...
		this.vollbildToggleBtn.addActionListener ( this );
		this.vollbildToggleBtn.setToolTipText ( "Umschalten zum Vollbildmodus" );
		this.vollbildToggleBtn.setHorizontalAlignment ( SwingConstants.LEFT );
		...
		addComponent ( screenControlePanel, gbl, konsolenPane, 0, 1, 2, 10, 1.0, 1.0, new Insets ( 0, 5, 5, 0 ) );
		addComponent ( screenControlePanel, gbl, this.schwarzerBildschirmBtn, 2, 1, 2, 1, 0, 0.2, new Insets ( 0, 5, 1,
				5 ) );
		addComponent ( screenControlePanel, gbl, this.vollbildToggleBtn, 2, 2, 2, 1, 0, 0.2, new Insets ( 0, 5, 1, 5 ) );
		addComponent ( screenControlePanel, gbl, this.spielPausierenBtn, 2, 3, 2, 1, 0, 0.2, new Insets ( 1, 5, 1, 5 ) );
		addComponent ( screenControlePanel, gbl, this.laufendsSpielBeendenBtn, 2, 4, 2, 1, 0, 0.2, new Insets ( 1, 5,
				1, 5 ) );
		addComponent ( screenControlePanel, gbl, this.einstellungenBtn, 2, 5, 2, 1, 0, 0.2, new Insets ( 1, 5, 1, 5 ) );
		addComponent ( screenControlePanel, gbl, this.programmBeenden, 2, 6, 2, 1, 0, 0.2, new Insets ( 1, 5, 1, 5 ) );

		addComponent ( screenControlePanel, gbl, new JLabel ( "Statistik:" ), 2, 7, 2, 1, 0, 0, new Insets ( 30, 5, 1,
				5 ) );
		addComponent ( screenControlePanel, gbl, this.uhrzeit, 2, 8, 2, 1, 0, 0, new Insets ( 1, 5, 1, 5 ) );
		addComponent ( screenControlePanel, gbl, this.aktuelleFragenZeit, 2, 9, 2, 1, 0, 0, new Insets ( 1, 5, 1, 5 ) );
		addComponent ( screenControlePanel, gbl, this.gesamtSpielZeit, 2, 10, 2, 1, 0, 0, new Insets ( 1, 5, 5, 5 ) );
	...
	public void actionPerformed (ActionEvent e) {
		if ( e.getSource () == this.sekZeit.timer ) {
			...
			this.adminRequest_setFiftyJokerAktiviert ( !this.fiftyJokerBtn.getToolTipText ()
					.equals ( "Joker verwenden" ) );
		}
			...
			// 1, "Neues Spiel aus dem Internet" ) );
		...
		else if ( e.getSource () == this.vollbildToggleBtn ) {
			if ( this.isFullScreen ) {
				Biblionaer.meineKonsole.println ( "Beende den Vollbildmodus ...", 3 );

				this.setFullscreen ( false );
				this.vollbildToggleBtn.setIcon ( this.fullscreenIcon );
				this.vollbildToggleBtn.setText ( "<html>Vollbildmodus</html>" );
				this.vollbildToggleBtn.setToolTipText ( "Umschalten zum Vollbildmodus" );
			}
			else {
				if ( this.device != null && this.device.isFullScreenSupported () ) {
					Biblionaer.meineKonsole.println ( "Wechsle in Vollbildmodus ...", 3 );

					this.setFullscreen ( true );
					this.vollbildToggleBtn.setIcon ( this.windowedIcon );
					this.vollbildToggleBtn.setText ( "<html>Beende Vollbildmodus</html>" );
					this.vollbildToggleBtn.setToolTipText ( "<html>beende Vollbildmodus</html>" );
				}
				else {
					Biblionaer.meineKonsole.println ( "Fehler: Das Ausgabegerät hat keine Vollbildunterstützung!", 2 );
				}
			}
		}
